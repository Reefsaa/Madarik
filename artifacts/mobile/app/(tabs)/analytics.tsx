...
const EXPENSE_CATEGORIES = [
  ...
  { label: 'Rent & Utilities', pct: 15, color: '#c7d2fe', amount: 'SAR 36,750' },
  { label: 'Other', pct: 10, color: '#e0e7ff', amount: 'SAR 24,500' },
];

const KPIS = [
  { label: 'Gross Margin', value: '32.4%', change: '+2.1%', positive: true },
  { label: 'Net Margin', value: '21.0%', change: '+1.8%', positive: true },
  { label: 'ROI', value: '18.6%', change: '+0.9%', positive: true },
  { label: 'Burn Rate', value: 'SAR 8,166/d', change: '-3.2%', positive: true },
];
    ...
    <ScrollView style={styles.screen} contentContainerStyle={{ paddingBottom: 120 }}>
      ...
      >
        ...
        <View style={styles.summaryRow}>
          {[
            { label: 'REVENUE', value: 'SAR 310K', change: '+12%', positive: true },
            { label: 'EXPENSES', value: 'SAR 245K', change: '+3.2%', positive: false },
            { label: 'NET PROFIT', value: 'SAR 65K', change: '+21%', positive: true },
          ].map((s) => (
            <View key={s.label} style={styles.summaryCard}>
              ...
              <View style={styles.changeBadge}>
                <Ionicons
                  name={s.positive ? 'arrow-up-outline' : 'arrow-down-outline'}
                  size={9}
                  color={s.positive ? '#86efac' : '#fca5a5'}
                />
                <Text style={[styles.changeText, { color: s.positive ? '#86efac' : '#fca5a5' }]}>
                  {s.change}
                </Text>
              </View>
            </View>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.chartLabels}>
            {['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'].map((m) => (
              <Text key={m} style={styles.chartLabel}>{m[0]}</Text>
            ))}
          </View>
        </View>
          ...
          {KPIS.map((k) => (
            <View key={k.label} style={styles.kpiCard}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 4 }}>
                <Ionicons
                  name={k.positive ? 'trending-up-outline' : 'trending-down-outline'}
                  size={11}
                  color={k.positive ? '#22c55e' : '#ef4444'}
                />
                <Text style={[styles.kpiChange, { color: k.positive ? '#22c55e' : '#ef4444' }]}>
                  {' '}{k.change}
                </Text>
              </View>
            </View>
        ...
        <View style={styles.card}>
          ...
          {EXPENSE_CATEGORIES.map((c) => (
            <View key={c.label} style={styles.expenseRow}>
              <View style={styles.expenseLeft}>
                <View style={[styles.expenseDot, { backgroundColor: c.color }]} />
...
const styles = StyleSheet.create({
  ...
  expenseLeft: { flexDirection: 'row', alignItems: 'center', width: 100 },
