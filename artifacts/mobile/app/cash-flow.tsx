...
export default function CashFlowScreen() {
  ...
  const CATEGORIES = [
    { labelKey: t('cashFlowClientRevenue'), amount: 'SAR 185,000', change: '+14%', positive: true,  icon: 'briefcase-outline' as const },
    { labelKey: t('cashFlowServiceFees'),   amount: 'SAR 72,000',  change: '+8%',  positive: true,  icon: 'card-outline' as const },
    { labelKey: t('cashFlowInvestments'),   amount: 'SAR 53,000',  change: '-2%',  positive: false, icon: 'trending-up-outline' as const },
  ];
  const OUTFLOWS = [
    { labelKey: t('cashFlowPayroll'),    amount: 'SAR 85,000', icon: 'people-outline' as const },
    { labelKey: t('cashFlowSuppliers'),  amount: 'SAR 67,000', icon: 'cube-outline' as const },
    { labelKey: t('cashFlowOperations'), amount: 'SAR 53,000', icon: 'cog-outline' as const },
    { labelKey: t('cashFlowOther'),      amount: 'SAR 40,000', icon: 'ellipsis-horizontal' as const },
  ];
    ...
    <View style={styles.container}>
      <LinearGradient colors={['#0f172a', '#1e1b4b']} style={[styles.header, { paddingTop: topPad + 8 }]}>
        <View style={[styles.headerRow, isRTL && { flexDirection: 'row-reverse' }]}>
          ...
          <Text style={styles.headerTitle}>{t('cashFlowForecastTitle')}</Text>
          <View style={{ width: 34 }} />
        </View>
        ...
        <View style={[styles.summaryRow, isRTL && { flexDirection: 'row-reverse' }]}>
          <View style={styles.summaryCard}>
            <Text style={styles.summaryLabel}>{t('cashFlowInflows')}</Text>
          ...
          <View style={styles.summaryCard}>
            <Text style={styles.summaryLabel}>{t('cashFlowOutflows')}</Text>
          ...
          <View style={styles.summaryDivider} />
          <View style={styles.summaryCard}>
            <Text style={styles.summaryLabel}>{t('cashFlowNetFlow')}</Text>
            <Text style={[styles.summaryValue, { color: '#86efac' }]}>SAR 65K</Text>
            <View style={styles.changePill}>
              <Ionicons name="trending-up-outline" size={9} color="#86efac" />
              <Text style={[styles.changeText, { color: '#86efac' }]}>{t('cashFlowPositive')}</Text>
            </View>
          </View>
        </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: bottomPad + 24 }}>
        ...
        <View style={styles.card}>
          <Text style={[styles.cardTitle, isRTL && { textAlign: 'right' }]}>{t('cashFlowChartTitle')}</Text>
          <View style={[styles.chartLegend, isRTL && { flexDirection: 'row-reverse' }]}>
            <View style={[styles.legendItem, isRTL && { flexDirection: 'row-reverse' }]}>
              ...
              <Text style={styles.legendText}>{t('cashFlowLegendInflows')}</Text>
            </View>
            <View style={[styles.legendItem, isRTL && { flexDirection: 'row-reverse' }]}>
              <View style={[styles.legendDot, { backgroundColor: '#fca5a5' }]} />
              <Text style={styles.legendText}>{t('cashFlowLegendOutflows')}</Text>
            </View>
          </View>
        ...
        {/* Inflows breakdown */}
        <View style={styles.card}>
          <Text style={[styles.cardTitle, { marginBottom: 14 }, isRTL && { textAlign: 'right' }]}>{t('cashFlowInflowSources')}</Text>
          {CATEGORIES.map((c) => (
            <View key={c.labelKey} style={[styles.flowRow, isRTL && { flexDirection: 'row-reverse' }]}>
              <View style={[styles.flowIcon, isRTL && { marginLeft: 10, marginRight: 0 }]}>
                <Ionicons name={c.icon} size={16} color="#4f46e5" />
              </View>
              <Text style={styles.flowLabel}>{c.labelKey}</Text>
              <Text style={styles.flowAmount}>{c.amount}</Text>
              <View style={[styles.changeBadge, { backgroundColor: c.positive ? '#f0fdf4' : '#fef2f2' }]}>
                <Text style={{ fontSize: 10, color: c.positive ? '#16a34a' : '#ef4444', fontFamily: 'Inter_600SemiBold' }}>{c.change}</Text>
              </View>
            </View>
          ))}
        </View>
        ...
        {/* Outflows breakdown */}
        <View style={styles.card}>
          <Text style={[styles.cardTitle, { marginBottom: 14 }, isRTL && { textAlign: 'right' }]}>{t('cashFlowOutflowCat')}</Text>
          {OUTFLOWS.map((o) => (
            <View key={o.labelKey} style={[styles.flowRow, isRTL && { flexDirection: 'row-reverse' }]}>
              <View style={[styles.flowIcon, { backgroundColor: '#fef2f2' }, isRTL && { marginLeft: 10, marginRight: 0 }]}>
                <Ionicons name={o.icon} size={16} color="#ef4444" />
              </View>
              <Text style={styles.flowLabel}>{o.labelKey}</Text>
        ...
        <LinearGradient colors={['#4f46e5', '#7c3aed']} style={styles.aiCard}>
          <View style={[styles.aiCardHeader, isRTL && { flexDirection: 'row-reverse' }]}>
            <Ionicons name="sparkles" size={14} color="#fff" />
            <Text style={styles.aiCardTitle}>{t('cashFlowModrikInsight')}</Text>
          </View>
          <Text style={[styles.aiCardText, isRTL && { textAlign: 'right' }]}>{t('cashFlowInsightBody')}</Text>
        </LinearGradient>
      </ScrollView>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  periodBtn: { paddingHorizontal: 16, paddingVertical: 6, borderRadius: 20, backgroundColor: 'rgba(255,255,255,0.1)' },
  ...
  flowIcon: { width: 34, height: 34, borderRadius: 10, backgroundColor: '#eef2ff', alignItems: 'center', justifyContent: 'center', marginRight: 10 },
