...
const PERIODS: Period[] = ['30D', '60D', '90D', '1Y'];
        ...
        <View style={styles.summaryRow}>
          {[
            ...
            <View key={s.l} style={styles.summaryCard}>
              ...
              <Text style={[styles.summaryChange, { color: s.pos ? '#86efac' : '#fca5a5' }]}>{s.c}</Text>
            </View>
          ))}
        </View>
      </LinearGradient>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.chartLabels}>
            {MONTHS.map((m, i) => <Text key={i} style={styles.chartLabel}>{m}</Text>)}
          </View>
        </View>
        ...
        <View style={styles.kpiGrid}>
          {KPIS.map((k) => (
            <View key={k.label} style={styles.kpiCard}>
              ...
              <Text style={[styles.kpiChange, { color: k.pos ? '#22c55e' : '#ef4444' }]}>{k.change}</Text>
            </View>
          ))}
        </View>
          ...
          {EXPENSES.map((e) => (
            <View key={e.label} style={styles.expenseRow}>
              ...
              <View style={styles.expenseBarBg}>
                <View style={[styles.expenseBar, { width: `${e.pct}%` as any, backgroundColor: e.color }]} />
              </View>
...
const styles = StyleSheet.create({
  ...
  summaryChange: { fontSize: 10, fontWeight: '600', marginTop: 2, fontFamily: 'Inter_600SemiBold' },
  ...
  periodText: { fontSize: 13, fontWeight: '600', color: '#6b7280', fontFamily: 'Inter_600SemiBold' },
  ...
  kpiChange: { fontSize: 11, fontWeight: '600', marginTop: 4, fontFamily: 'Inter_600SemiBold' },

  expenseRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 12 },
  expenseDot: { width: 8, height: 8, borderRadius: 4, marginRight: 8 },
  expenseLabel: { fontSize: 12, color: '#374151', width: 72, fontFamily: 'Inter_400Regular' },
  expenseBarBg: { flex: 1, height: 6, backgroundColor: '#f3f4f6', borderRadius: 3, marginHorizontal: 8 },
